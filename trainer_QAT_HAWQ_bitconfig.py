...
import wandb
# from quantization_utils_classengine.iresnet import quantize_model_mix, quantize_model
...
这份代码主要是，直接加载HAWQ论文里得到的结论bit位数，查看效果，包含模型大小，flops，acc
"""
...
parser.add_argument('--bit-config', type=str, default='bit_config_resnet18_uniform8_lxy', help='Bit configuration for quantization')

best_prec1 = 0
args = parser.parse_args()
os.environ["WANDB_API_KEY"] = "1df731212a55e7e0f9e8e6c3a31983590d3c19ca"
wandb.init(project="ResNet20-QAT-CIFAR10",name=f"default_{args.arch}_stage4_qat_cifar10_hawq_{args.bit_config}")
...
def main():
    ...
    bit_config = bit_config_dict.get(args.bit_config, None)
    if bit_config is None:
        raise ValueError(f"Invalid bit configuration: {args.bit_config}")
    model = resnet.quantize_model_mix1(model, bit_config)
    print(20*'-','quantized model')
    print(model)
    print(50*'-')
